...
namespace psx::video {
	...
	void Vram::Upload() {
		auto gl_context = GetCurrentGLContext();
		gl_context->BindTexture({ .type = GL_TEXTURE_2D, .handle = m_texture_id });
	...
	void Vram::CreateBuffer() {
		...
		gl_context->BindTexture({ .type = GL_TEXTURE_2D, .handle = m_blit_texture });
	...
	void Vram::MapBuffer() {
		auto gl_context = GetCurrentGLContext();
		gl_context->BindBuffer(GL_PIXEL_UNPACK_BUFFER, m_buffer_id);

		m_buffer_ptr = (u8*)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, (GLintptr)0, VRAM_SIZE_BYTES,
			GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT |
			GL_MAP_READ_BIT | GL_MAP_WRITE_BIT);

		gl_context->BindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
	}
