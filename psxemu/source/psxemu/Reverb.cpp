...
#define MULT(l, r) (((l) * (r)) >> 15)
	...
	void SPU::ReverbWrite(i16 value, i32 offset) {
		...
		const u32 MBASE = (u32)m_regs.m_reverb.work_area_start << 3;
		if (final_address < MBASE) {
			u32 distance = MBASE - final_address;
			final_address = REVERB_BUFFER_END - distance;
		}
		else if (final_address > REVERB_BUFFER_END) {
			u32 distance = final_address - REVERB_BUFFER_END;
			final_address = MBASE + distance;
		}
